...
│   ├── vite.config.js
│   └── package.json
├── middle/          # 第二级：既是 remote 又是 host（端口 5174）
│   ├── src/
│   │   ├── Home.vue
│   │   ├── router/
│   │   │   └── index.js
...
- **作为 Host**：通过路由加载 remote 组件
- **作为 Remote**：暴露 `./Home` 组件
- **路由配置**：
  - `/` → Home 组件
...
- **middle**: `cd middle && npm run dev`（独立开发，使用 mock 或真实 remote）
- **shell**: `cd shell && npm run dev`（独立开发，使用 mock 或真实 middle）

### 联调模式

使用 `./start-dev.sh` 脚本，会自动：
1. 构建 remote 和 middle
2. 启动 remote preview (5173)
3. 启动 middle preview (5174)
4. 启动 shell dev (5175)
